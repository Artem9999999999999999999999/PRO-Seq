...
```{r}
Counts <- read.delim("data/featureCounts_r6_53.csv", header = TRUE, row.names = 1, sep = ",")
...
```{r}
file_path_CHD1 <- "results/diff_expr_CHD1(r6.53).csv"
file_path_S2 <- "results/diff_expr_S2(r6.53).csv"
...
```{r}
annotation_file <- "data/dmel-all-r6.53.gtf.gz"
...
ggplot(filtered_counts_CHD1.df, aes(x = group, y = log2FoldChange, fill = group)) +
  geom_boxplot() +
  labs(title = "Boxplot of log2FoldChange for DE genes of Chd1(r6.53)", x = "Group", y = "log2FoldChange") +
...
ggplot(filtered_counts_CHD1.df, aes(x = log2FoldChange, color = group)) +
  ...
  labs(title = "Density Plot of log2FoldChange for DE genes of  Chd1(r6.53)", x = "log2FoldChange", y = "Density") +
...
plot_path_s2 <- file.path(figures_folder, "MA_Plot_S2(r6.53).png")
plot_path_chd1 <- file.path(figures_folder, "MA_Plot_Chd1(r6.53).png")
plot_path_PCA <- file.path(figures_folder, "PCA_Plot(r6.53).png")
boxplot_path <- file.path(figures_folder, "Boxplot_DE_Chd1(r6.53).png")
density_plot_path <- file.path(figures_folder, "Density_plot_DE_Chd1(r6.53).png")
...
ggplot(pca_data, aes(x = PC1, y = PC2, color = condition_all, shape = type)) +
  geom_point(size = 4) +
  labs(title = "PCA plot for S2(r6.53) and CHD1(r6.53)") +
  xlab(paste0("PC1: ", round(100 * pca_data$percentVar[1], 1), "% variance")) +
  ylab(paste0("PC2: ", round(100 * pca_data$percentVar[2], 1), "% variance")) +
  scale_x_continuous(name = "PC1", labels = scales::percent_format(scale = 1)) +
  scale_y_continuous(name = "PC2", labels = scales::percent_format(scale = 1)) +
...
ggplot(filtered_counts_CHD1.df, aes(x = group, y = log2FoldChange, fill = group)) +
  geom_boxplot() +
  labs(title = "Boxplot of log2FoldChange for DE CHD1(r6.53)", x = "Group", y = "log2FoldChange") +
  scale_fill_manual(values = color_palette, name = "Group") +
  theme_minimal() +
  ylim(-1, 1)
dev.off()
...
ggplot(filtered_counts_CHD1.df, aes(x = log2FoldChange, color = group)) +
  geom_density(alpha = 0, size = 1) + 
  labs(title = "Density Plot of log2FoldChange for DE CHD1(r6.53)", x = "log2FoldChange", y = "Density") +
  scale_color_manual(values = color_palette, name = "Group") +
  theme_minimal() +
  xlim(-1, 1)
dev.off
