...
```{r}
Counts <- read.delim("data_r6.52/result_featurecounts.csv", header = TRUE, row.names = 1, sep = ",")
...
```{r}
annotation_file <- "data_r6.52/dmel-all-r6.52.gtf.gz"
...
filtered_counts_CHD1.df <- filtered_counts_CHD1.df[order(filtered_counts_CHD1.df$log2FoldChange, decreasing = T), ]
dim(filtered_counts_CHD1.df)
head(filtered_counts_CHD1.df, 10)
```


```{r}
file_path_CHD1 <- "results_r6.52/diff_expr_CHD1.csv"
...
```{r}
figures_folder <- "figures_r6.52"

if (!file.exists(figures_folder)) {
  dir.create(figures_folder)
}

boxplot_path_up <- file.path(figures_folder, "Boxplot_DE_chd1_up(r6.52).png")
boxplot_path_down <- file.path(figures_folder, "Boxplot_DE_chd1_down(r6.52).png")
density_plot_path_up <- file.path(figures_folder, "Density_plot_DE_chd1_up(r6.52).png")
density_plot_path_down <- file.path(figures_folder, "Density_plot_DE_chd1_down(r6.52).png")
...
ggplot(filtered_counts_CHD1_up.df, aes(x = log2FoldChange, color = group)) +
  geom_density(alpha = 0, size = 1) + 
  labs(title = paste("Density Plot of log2FoldChange for DE genes of  Chd1 UP (N =", nrow(filtered_counts_CHD1_up.df), ")", sep = ""), x = "log2FoldChange", y = "Density") +
  scale_color_manual(values = color_palette, name = "Group") +
  theme_minimal() +
  xlim(0, 2)
dev.off()
...
ggplot(filtered_counts_CHD1_down.df, aes(x = log2FoldChange, color = group)) +
  geom_density(alpha = 0, size = 1) + 
  labs(title = paste("Density Plot of log2FoldChange for DE genes of  Chd1 DOWN (N =", nrow(filtered_counts_CHD1_down.df), ")", sep = ""), x = "log2FoldChange", y = "Density") +
  scale_color_manual(values = color_palette, name = "Group") +
  theme_minimal() +
  xlim(-2, 0)
dev.off()
